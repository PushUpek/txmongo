...
class Database(object):
    ...
    def __init__(
        self, factory, database_name, write_concern=None, codec_options=None
    ):
    ...
    def command(
        ...
        **kwargs
        ...
        def on_proto(protocol):
            ...
            return self._send_command_to_proto(
                protocol,
                command,
                check=check,
    ...
    def _send_command_to_proto(
        ...
        def on_reply(raw_reply):
            check_deadline(_deadline)
            response = raw_reply.documents[0].decode(
        ...
        query = Query(
            collection=str(self) + '.$cmd', query=command, flags=flags
        )
    ...
    def collection_names(self, _deadline=None):
        ...
        return (
            self['system.namespaces'].find(_deadline=_deadline).addCallback(ok)
        )
    ...
    def authenticate(self, name, password, mechanism='DEFAULT'):
        ...
        if not isinstance(password, (bytes, unicode)):
            raise TypeError(
                'TxMongo: password must be an instance of basestring.'
            )
